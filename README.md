...
Stop trading performance for convenience. **vi/validation** delivers **29x to 131x speedups**
compared to standard Laravel validation, depending on rule complexity and dataset size.

<!-- Generated by `php tests/render-benchmark-table.php` from benchmark-results/latest.json - do not hand-edit the table below. -->
| Scenario | Rows | FastValidator 🚀 | Laravel Validator | Speedup | Throughput |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Simple Rules** | 1,000 | **0.0018s** | 0.1798s | **99.8x** | ~554,790 rows/s |
| **Simple Rules** | 10,000 | **0.0190s** | 1.83s | **96.7x** | ~527,485 rows/s |
| **Simple Rules** | 100,000 | **0.1957s** | 18.33s | **93.7x** | ~510,889 rows/s |
| **Medium Rules** | 1,000 | **0.0024s** | 0.3204s | **130.9x** | ~408,597 rows/s |
| **Medium Rules** | 10,000 | **0.0289s** | 3.13s | **108.1x** | ~345,904 rows/s |
| **Medium Rules** | 100,000 | **0.2874s** | 33.63s | **117.0x** | ~347,953 rows/s |
| **Complex Rules** | 1,000 | **0.0098s** | 0.3634s | **37.2x** | ~102,347 rows/s |
| **Complex Rules** | 10,000 | **0.1020s** | 3.80s | **37.3x** | ~98,072 rows/s |
| **Complex Rules** | 100,000 | **1.10s** | 37.64s | **34.3x** | ~91,153 rows/s |
| **ETL Import** | 1,000 | **0.0139s** | 0.4614s | **33.2x** | ~72,062 rows/s |
| **ETL Import** | 10,000 | **0.1481s** | 4.29s | **29.0x** | ~67,510 rows/s |
| **ETL Import** | 100,000 | **1.43s** | 43.30s | **30.2x** | ~69,694 rows/s |

> *"Complex Rules" and "ETL Import" include nested fields, regex, conditional requirements, and
> type checks - see [Benchmarks](#-benchmarks) below for what each scenario/column means,
> methodology, how to reproduce these numbers, and the raw data
> (`benchmark-results/latest.json`) they're generated from. Measured on PHP 8.5.9,
> illuminate/validation v11.47.0; your numbers will vary by hardware and PHP/OPcache/JIT
> configuration - that's the point of making this reproducible.*
...
The numbers in [Performance at a Glance](#-performance-at-a-glance) come from a reproducible
suite, not hand-maintained figures — see issue
[#9](https://github.com/ahmed-nour-dev/vi-validation/issues/9).

### Reproduce it

```bash
composer install
composer bench          # full suite: 4 scenarios x 4 variants x 1k/10k/100k rows
composer bench:quick    # a much faster subset, useful while iterating locally
php tests/run-benchmarks.php --large   # also include a 1,000,000-row pass (slow)
php tests/run-benchmarks.php --help    # all options (--scenarios, --sizes, --variants, --out, ...)
```

Results are written as machine-readable JSON to `benchmark-results/latest.json` (the committed,
traceable snapshot the README table above is generated from) and, per run, to a timestamped
`benchmark-results/run-<timestamp>.json` (gitignored history). Regenerate the markdown table
itself with:

```bash
php tests/render-benchmark-table.php   # prints the "Performance at a Glance" table for latest.json
```

### What's measured

Every scenario's rules are defined once (as a Laravel-style rules array) and fed to both
Laravel's own validator and vi/validation, so there is exactly one rule definition to keep in
sync. Four variants are measured per scenario/size:

| Variant | What it measures |
| :--- | :--- |
| `laravel` | Standalone `Illuminate\Validation\Factory` — the baseline. |
| `engine` | `ValidatorEngine::validate()` called directly, bypassing `SchemaValidator`'s own dispatch overhead — isolates raw rule-execution cost. |
| `compiled` | Full `SchemaValidator` public API with native compilation disabled — the real overhead on top of `engine` that the public API costs. |
| `native` | Full `SchemaValidator` with native compilation enabled. Reported separately as a single cold call (pays `file_exists()` + `require()`) and steady-state warm trials. |

Four scenarios, in increasing complexity — `simple`, `medium`, `complex`, and `etl` (a realistic
~14-dirty-row-rate bulk contact-import shape, not just a microbenchmark) — are defined in
[`tests/Benchmarks/ScenarioRegistry.php`](tests/Benchmarks/ScenarioRegistry.php). `complex` and
`etl` intentionally use rules (`in`, `regex`, `required_if`) outside the
[native compilation compatibility contract](#native-compilation-compatibility-contract), so
`native` correctly reports as unsupported/falls back to `compiled` for those two — that's the
documented contract working as intended, not a benchmark bug.

Also measured, once per scenario rather than per row: `SchemaBuilder::compile()` time,
`NativeCompiler::compile()` (pure codegen) time, and codegen+disk-write time.

### Methodology

- **Deterministic data**: every row is a pure function of its index (no PRNG), so a run is
  reproducible byte-for-byte given the same scenario/size.
- **Warmup + median-of-N**: a configurable number of warmup passes are discarded before timing;
  reported numbers are the median of the remaining trials (see `duration_seconds` in the JSON
  for the full min/max/mean/median/stddev spread).
- **Process isolation for memory**: `memory_get_peak_usage()` can't be reset mid-process, so
  each (scenario, variant, size) combination runs in its own PHP subprocess for a clean peak
  reading — this is also why a full `composer bench` run takes noticeably longer than a single
  in-process microbenchmark.
- **Environment capture**: PHP version, OS, CPU model/cores, memory limit, OPcache/JIT
  configuration, `illuminate/validation` version, and git commit are recorded into the results
  JSON's `environment` block (captured from inside a worker subprocess, so `opcache.enable_cli`
  reflects what the numbers were actually measured under).

### CI regression gate

A separate, much lighter check runs on every CI build:
[`tests/Unit/Performance/RegressionBenchmarkTest.php`](tests/Unit/Performance/RegressionBenchmarkTest.php)
(`./vendor/bin/phpunit --group performance`). Shared CI runners have noisy, variable CPU speed,
so instead of an absolute wall-clock threshold, it measures Laravel and vi/validation's native
path back-to-back in the same process on the same small dataset — so both feel identical runner
noise — and fails the build only if the speedup ratio between them collapses below a
conservative floor (3x; normal speedups are 17-34x). It also asserts Laravel/vi parity on a
sample of rows first, so a "faster" build that's actually a correctness regression can't pass by
accident.
